...
% 记录辨识元信息
sol.method = method;
sol.data_file = path_to_data;
sol.idx = idx;
sol.timestamp = datestr(now, 'yyyy-mm-dd_HH-MM-SS');

% 自动保存辨识结果到 .mat 文件
saveIdentificationResult(sol, method, path_to_data);
...
function [Tau, Wb] = buildObservationMatrices(idntfcnTrjctry, baseQR)
    ...
    for i = 1:nSamples
        ...
        if baseQR.motorDynamicsIncluded
            ...
            g=[0 0 9.8065]';
            Yi = standard_regressor_marvin(q, qd, q2d,g);
        end
...
% ======================================================================
% 自动保存辨识结果到 results/ 目录
% 文件名格式: sol_<方法>_<时间戳>.mat
% ======================================================================
function saveIdentificationResult(sol, method, path_to_data)
    resultDir = fullfile(fileparts(mfilename('fullpath')), 'results');
    if ~exist(resultDir, 'dir')
        mkdir(resultDir);
    end
    
    [~, dataName] = fileparts(path_to_data);
    fileName = sprintf('sol.mat');
    savePath = fullfile(resultDir, fileName);
